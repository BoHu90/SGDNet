...
class IQASolverAva(object):
    ...
    def __init__(self, config, path, train_idx, test_idx):
        ...
        self.model = torch.nn.DataParallel(model, device_ids=[0, 1]).cuda()#多gpu并行计算
        if self.root == 'ava':
    ...
    def train(self, i, checkpoint_dir, config):
        ...
        # if os.path.exists('/home/hb/zhengjia/SGDNet/ava_save_models/epoch2_model.pth'):
        #     checkpoint_true = torch.load('/home/hb/zhengjia/SGDNet/ava_save_models/epoch2_model.pth')
        #     self.model.load_state_dict(checkpoint_true['final_model'])
        #     self.optimizer.load_state_dict(checkpoint_true['optimizer'])
        #     start_epoch = checkpoint_true['epoch'] + 1
        #     print('找到模型，从epoch_{}开始训练'.format(start_epoch))
        # else:
        #     start_epoch = 1
        #     print('没有找到模型，从epoch_1开始训练')
        ...
        for epoch in range(1, (self.epochs+1)):
            ...
            # 保存最后一个epoch的模型
            final_model_name = os.path.join(checkpoint_dir, 'model.pth'.format(epoch))
            torch.save({
            ...
            print('%d\t%4.3f\t\t%4.4f\t\t%4.4f\t\t%4.4f\t\t%4.4f\t\t%4.4f' %
                  (epoch, sum(epoch_loss) / len(epoch_loss), train_srcc, test_srcc, test_plcc, test_krocc,
                   test_RMSE))
        ...
        best_srcc, best_plcc, best_krcc, best_rmse, int(best_epoch)))
