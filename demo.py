...
class IQASolver(object):
    ...
    def train(self, i, checkpoint_dir, config):
        ...
        # if os.path.exists("/ava_save_models/"):
        #     checkpoint_true = torch.load('模型位置')
        #     self.model.load_state_dict(checkpoint_true['model'])
        ...
        for epoch in range(0, self.epochs):
            ...
            # 保存最后一个epoch的模型
            final_model_name = os.path.join(checkpoint_dir, 'epoch{}_model.pth'.format(epoch+1))
            torch.save({
