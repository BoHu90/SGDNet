...
class SEM(nn.Module):
    def __init__(self):
        ...
        self.conv3 = BasicConv(32, 64, kernel_size=3, bias=True, stride=1, padding=1, bn=False, relu=False)
...
class Net(nn.Module):
    ...
    def __init__(self, root, pvt, dims=[64, 128, 320, 512], config=None):
    ...
    def forward(self, data):
        ...
        # print(local_features.shape,"局部")
        # 语义引导模块
        global_feature = self.sem(x)  # [N, 64, 96, 96]
        # print(global_feature.shape,"全局")
        ...
        #X = torch.cat((local_features, global_feature), 1)
        ...
        # print(q[0])
        if self.root == 'ava':
            mos = dist_to_mos(q) # print(mos.size())  #torch.Size([32, 1])
            return q, mos
        else:
            return q
