...
class IQASolver(object):
    ...
    def __init__(self, config, train_path, test_path, train_idx, test_idx):
        ...
        self.model = Net(pvt, dims=[64, 128, 320, 512], config=config).cuda()
        ...
        train_loader = data_loader.DataLoader(config.train_dataset, train_path, train_idx, config.patch_size,
                                              config.train_patch_num, batch_size=config.batch_size, istrain=True)
        test_loader = data_loader.DataLoader(config.test_dataset, test_path, test_idx, config.patch_size,
                                             config.test_patch_num, batch_size=1,
                                             istrain=False)
    ...
    def train(self, i, checkpoint_dir, config):
        ...
        for epoch in range(self.epochs):
            ...
            # 保存每一个epoch的模型
            # model_name = os.path.join(checkpoint_dir, 'epoch{}_model.pth'.format(epoch+1))
            # torch.save({
            #     'final_model': self.model.state_dict(),
            #     'optimizer': self.optimizer.state_dict(),
            # }, model_name)
