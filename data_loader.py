...
class DataLoader(object):
    ...
    def __init__(self, dataset, path, img_indx, patch_size, patch_num, batch_size=1, istrain=True):
        ...
        if (dataset == 'csiq') | (dataset == 'tid2013'):
            # Train transforms
            if istrain:
        ...
        elif (dataset == 'live') | (dataset == 'livec'):
            # Train transforms
            if istrain:
                transforms = torchvision.transforms.Compose([
                    # torchvision.transforms.RandomHorizontalFlip(),
            ...
            if istrain:
                transforms = torchvision.transforms.Compose([
                    # torchvision.transforms.RandomHorizontalFlip(),
                    torchvision.transforms.Resize((1024, 683)),
                    torchvision.transforms.RandomCrop(size=patch_size),
                    torchvision.transforms.ToTensor(),
                    torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                                     std=(0.229, 0.224, 0.225))])
        ...
        if dataset == 'live':
            ...
            self.data = folders.AVAFolder(istrain=self.istrain, root=path, index=img_indx, transform=transforms, patch_num=patch_num)
        elif dataset == 'tests':
            self.data = folders.TestFolder(transform=transforms, patch_num=patch_num)
        elif dataset == 'AADB':
            self.data = folders.AADBFolder(
                istrain=self.istrain, root=path, transform=transforms, patch_num=patch_num)


    def get_data(self):
        if self.istrain:
            print("训练", self.batch_size)
            dataloader = torch.utils.data.DataLoader(
                self.data, batch_size=self.batch_size, shuffle=True, num_workers=4)
        else:
            print("测试", self.batch_size)
            dataloader = torch.utils.data.DataLoader(
